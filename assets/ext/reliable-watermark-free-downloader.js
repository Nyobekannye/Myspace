...
(function installReliableBrandAiDownloader() {
  ...
  // Whempy v1.9: remember verified downloads across service-worker restarts so that
  // re-opening a chat does not auto-download the same videos again. Manual "Download all as
  // ZIP" intentionally ignores this list (the user asked for a full archive).
  const DOWNLOADED_KEY = 'whempy_downloaded';
  const DOWNLOADED_MAX = 3000;
  const downloadedKeys = new Set();
  let downloadedLoaded = new Promise(resolve => {
    try {
      chrome.storage.local.get(DOWNLOADED_KEY, result => {
        void chrome.runtime.lastError;
        (Array.isArray(result?.[DOWNLOADED_KEY]) ? result[DOWNLOADED_KEY] : []).forEach(k => downloadedKeys.add(String(k)));
        resolve();
      });
    } catch { resolve(); }
  });
  let persistTimer = 0;
  function rememberDownloaded(...keys) {
    keys.filter(Boolean).forEach(k => downloadedKeys.add(String(k)));
    clearTimeout(persistTimer);
    persistTimer = setTimeout(() => {
      const list = Array.from(downloadedKeys).slice(-DOWNLOADED_MAX);
      chrome.storage.local.set({ [DOWNLOADED_KEY]: list }, () => void chrome.runtime.lastError);
    }, 300);
  }
  function wasDownloaded(video) {
    return downloadedKeys.has(videoIdentity(video)) || downloadedKeys.has(String(video?.url || '').trim());
  }
  ...
  function videoFilename(video) {
    return `Whempy_Videos/${safeVideoBaseName(video)}.mp4`;
  }

  function zipFilename(videoCount) {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    return `Whempy_Videos/Whempy_${videoCount}_Videos_${timestamp}.zip`;
  }
  ...
  async function startAttempt(job) {
    ...
    console.info(`[Whempy Downloader] Attempt ${job.attempt}/${MAX_ATTEMPTS} started`, {
      id,
  ...
  async function startVideoZip(videos, tabId, pageUrl) {
    ...
    try {
      ...
      console.info('[Whempy Downloader] Video ZIP started', {
        downloadId,
        videoCount: videos.length,
        filename
      });
  ...
  async function failJob(id, job, reason) {
    await deleteJob(id);
    releaseLock(job);
    console.error('[Whempy Downloader] Full MP4 could not be verified:', reason);
    notifyTab(job, {
  ...
  async function completeJob(id, job, actualBytes, expectedBytes) {
    await deleteJob(id);
    releaseLock(job);
    console.info('[Whempy Downloader] Full MP4 verified:', {
      id,
      actualBytes,
      expectedBytes
    });
    rememberDownloaded(job.key, job.url);
    notifyTab(job, {
  ...
  async function handleTerminalDownload(id, state) {
    ...
    try {
      ...
      if (isIncomplete(actualBytes, expectedBytes)) {
        ...
        console.warn('[Whempy Downloader] Partial MP4 detected; retrying:', reason);
      ...
      console.error('[Whempy Downloader] Verification stopped safely:', error);
      if (job) {
        await deleteJob(id);
        releaseLock(job);
      }
  ...
  async function finishZipJob(id, job, error = '') {
    ...
    if (error) {
      console.error('[Whempy Downloader] Video ZIP download failed:', error);
      notifyTab(job, {
    ...
    console.info('[Whempy Downloader] Video ZIP completed:', {
      id,
      videoCount: job.videoCount,
      filename: job.filename
    });
    (job.videos || []).forEach(v => rememberDownloaded(v?.key, v?.url));
    notifyTab(job, {
  ...
  async function handleTerminalZipDownload(id, state) {
    ...
    try {
      ...
      console.error('[Whempy Downloader] ZIP bookkeeping stopped safely:', error);
      if (job) {
        await deleteZipJob(id);
        releaseVideoLocks(job.videos);
        await releaseZipBlob(job.blobUrl);
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (!message.manual && wasDownloaded(message.video || {})) { sendResponse({ ok: true, downloaded: false, reason: 'Already downloaded' }); return true; }
    ...
    void (async () => {
      try {
        ...
        const job = {
          key,
          url,
          filename: `Whempy_Videos/${safeName}.mp4`,
          expectedBytes: expectedBytesFromVideo(video),
          attempt: 1,
          resumeCount: 0,
          tabId: sender?.tab?.id,
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    void (async () => {
      await downloadedLoaded;
      let videos = highestQualityVideos(message.videos);
      if (!message.manual) {
        const fresh = videos.filter(video => !wasDownloaded(video));
        if (!fresh.length && videos.length) {
          sendResponse({ ok: true, downloaded: false, mode: 'skipped', reason: 'All videos already downloaded', videoCount: 0 });
          return;
        }
        videos = fresh;
      }
      ...
      // Mobile browsers (Kiwi etc.) lack chrome.offscreen: fall back to individual
      // downloads for any batch size instead of failing the whole batch.
      const offscreenAvailable = typeof chrome.offscreen?.createDocument === 'function';
      if (videos.length <= INDIVIDUAL_DOWNLOAD_LIMIT || !offscreenAvailable) {
        const results = await Promise.all(
