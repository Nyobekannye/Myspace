(function () {
  ...
  function updateMiniBadge(progress = {}) {
    ...
    badge.innerHTML = `
      ...
      <span>⚡ <b>Whempy ZIP</b> ${progress.isPaused ? 'Paused' : 'Downloading'}: <b>${percent}%</b></span>
    `;
  }
  ...
  function ensureZipOverlay() {
    ...
    appendElement(badgeRow, 'div', {
      html: '<span style="color:#818cf8;margin-right:4px;">⚡</span> Bulk Downloader by <b>Whempy</b>',
      style: [
        'display:inline-flex',
        'align-items:center',
        'padding:4px 12px',
    ...
    appendElement(footer, 'span', {
      text: '⚡ Bulk Downloader by Whempy',
  ...
  // MAX MODE: auto-download is OFF by default. It can be enabled from the popup Settings
  // (chrome.storage.local.autoDownload === true). While OFF, videos are still collected into
  // the pool so "Download all" in the popup keeps working.
  let isAutoDownloadEnabled = false;

  chrome.storage.local.get(['brandai_bulk_mode', 'autoDownload'], res => {
    isBulkModeActive = Boolean(res?.brandai_bulk_mode);
    isAutoDownloadEnabled = res?.autoDownload === true;
    isStorageInitialized = true;
    updateBulkDock();
    if (!isBulkModeActive && isAutoDownloadEnabled) {
  ...
  chrome.storage.onChanged.addListener(changes => {
    ...
    if (changes.autoDownload) {
      isAutoDownloadEnabled = changes.autoDownload.newValue === true;
      isStorageInitialized = true;
      if (isAutoDownloadEnabled && !isBulkModeActive) {
        // Switched ON: flush everything that was held back while OFF.
        for (const v of extractedVideosPool.values()) {
          const key = String(v.vid || v.url);
          if (seenMedia.has(key) || savedKeys.has(key)) continue;
          seenMedia.add(key); seenMedia.add(String(v.url));
          queuePageVideo({ ...v, pageUrl: v.pageUrl || location.href, prompt: v.prompt || v.topicTitle || v.title || '' });
        }
        for (const [pageUrl] of pendingPageBatches) void flushPageBatch(pageUrl);
      }
    }
  });
  ...
  async function flushPageBatch(pageUrl, force = false, isManualTrigger = false) {
    ...
    if ((isBulkModeActive || !isAutoDownloadEnabled) && !isManualTrigger) {
      updateBulkDock();
      return;
    }
    ...
    try {
      ...
      if (!response?.ok) {
        ...
        if (!isCancelled) {
          ...
          console.warn('[Whempy Downloader] Batch download failed:', response?.error || response?.reason);
        } else {
      ...
      if (!isCancelled) {
        ...
        console.warn('[Whempy Downloader] Batch download unavailable:', error);
      } else {
        for (const v of videos) {
          const k = mediaKey(v);
          const u = String(v?.url || '');
  ...
  function stopAutoScrollChat(completed = false) {
    ...
    if (zipAfterScan) {
      zipAfterScan = false;
      if (completed && extractedVideosPool.size) {
  ...
  function triggerManualBulkDownload() {
    ...
    if (!filtered.length) return { ok: false, reason: 'No videos collected yet. Scan the chat first.' };
    ...
    return { ok: true, started: true, count: filtered.length };
  }
  ...
  function updateBulkDock(completed = false) {
    ...
    if (!host) return;
    if (!isBulkModeActive && !isAutoScrolling) {
      if (dock) dock.remove();
      return;
    }
    if (!dock) {
    ...
    const state = isAutoScrolling ? 'scan' : (completed ? 'done' : 'idle');
    if (dock.dataset.state === state && dock.dataset.count === String(count)) return;
    dock.dataset.state = state;
    dock.dataset.count = String(count);

    const label = isAutoScrolling
      ? `Scanning chat… ${count} video${count === 1 ? '' : 's'}`
      : `${count} video${count === 1 ? '' : 's'} ready — pilih di MAX MODE`;
    dock.innerHTML = `
      <span class="wh-dock-label">${label}</span>
      <button type="button" class="wh-dock-btn" id="brandai-dock-scroll-btn">${isAutoScrolling ? 'Stop' : 'Scan chat'}</button>
      <button type="button" class="wh-dock-btn wh-dock-close" id="brandai-dock-close-btn" title="Close" aria-label="Close">✕</button>
    `;
    dock.querySelector('#brandai-dock-scroll-btn').addEventListener('click', e => {
      e.preventDefault(); e.stopPropagation();
      if (isAutoScrolling) stopAutoScrollChat(false); else startAutoScrollChat();
    });
    dock.querySelector('#brandai-dock-close-btn').addEventListener('click', e => {
      e.preventDefault(); e.stopPropagation();
      if (isAutoScrolling) stopAutoScrollChat(false);
      setBulkMode(false);
    });
  }
      ...
      toast.innerHTML = `
        ...
        <div style="display:flex;flex-direction:column;gap:2px;">
          ...
          <span style="color:#94a3b8;font-size:11.5px;max-width:280px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
            ${String(videoName || 'Saved to Downloads/Whempy_Videos').replace(/</g, '&lt;')}
          </span>
        </div>
      `;
  ...
  window.addEventListener('DOLA_VIDEO_EXTRACTED', event => {
    ...
    if (isBulkModeActive || !isAutoDownloadEnabled) {
      updateBulkDock();
      return; // No auto-download while Bulk Mode is on or Auto-download is OFF (default).
    }
  ...
  // ---- Whempy popup API (status + scan + per-video download) ----
  const savedKeys = new Set();
  const savingKeys = new Set();
  function popupVideoList() {
    const out = [];
    for (const [key, v] of extractedVideosPool.entries()) {
      const url = String(v?.url || '');
      if (!url || cancelledMedia.has(key) || cancelledMedia.has(url)) continue;
      const saved = savedKeys.has(key) || savedKeys.has(url) || savedKeys.has(String(v?.vid || ''));
      const saving = !saved && (savingKeys.has(key) || savingKeys.has(url));
      out.push({
        key, url,
        title: String(v?.prompt || v?.topicTitle || v?.title || '').trim().slice(0, 140),
        poster: String(v?.poster_url || v?.poster || ''),
        width: Number(v?.width) || 0, height: Number(v?.height) || 0,
        duration: Number(v?.duration) || 0,
        bytes: Number(v?.expectedBytes) || 0,
        saved, saving
      });
    }
    // terbaru di atas (urutan penemuan = urutan chat dari atas ke bawah)
    return out.reverse();
  }
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message?.type === 'WHEMPY_GET_STATUS') {
      sendResponse({ ok: true, found: extractedVideosPool.size, saved: savedKeys.size, collecting: isAutoScrolling, autoDownload: isAutoDownloadEnabled, videos: popupVideoList() });
      return true;
    }
    if (message?.type === 'WHEMPY_SCAN') {
      if (isAutoScrolling) { stopAutoScrollChat(false); sendResponse({ ok: true, started: false, stopped: true }); return true; }
      zipAfterScan = false;
    ...
    if (message?.type === 'WHEMPY_DOWNLOAD_ONE') {
      const key = String(message.key || '');
      const video = extractedVideosPool.get(key) || Array.from(extractedVideosPool.values()).find(v => String(v?.url || '') === key || String(v?.vid || '') === key);
      if (!video) { sendResponse({ ok: false, reason: 'Video tidak ditemukan — scan ulang.' }); return true; }
      const k = mediaKey(video), u = String(video.url || '');
      seenMedia.delete(k); seenMedia.delete(u);
      cancelledMedia.delete(k); cancelledMedia.delete(u);
      savingKeys.add(k); savingKeys.add(u);
      try {
        chrome.runtime.sendMessage({ type: 'BRANDAI_RAW_VIDEO_DOWNLOAD', manual: true, video: { ...video, pageUrl: location.href } }, res => {
          void chrome.runtime.lastError;
          if (!res?.ok) { savingKeys.delete(k); savingKeys.delete(u); }
          sendResponse(res && typeof res === 'object' ? res : { ok: false, reason: 'Tidak ada respons dari downloader' });
        });
      } catch (error) {
        savingKeys.delete(k); savingKeys.delete(u);
  ...
  chrome.runtime.onMessage.addListener(message => {
    ...
    if (message?.type === 'BRANDAI_RAW_VIDEO_DOWNLOAD_FAILED') {
      savingKeys.delete(String(message.key || '')); savingKeys.delete(String(message.url || ''));
      releaseVideo(message);
      console.warn('[Whempy Downloader] Full video could not be verified:', message.error);
      showFailureToast(message);
      return;
    }

    if (message?.type === 'BRANDAI_RAW_VIDEO_DOWNLOAD_VERIFIED') {
      if (message.key) { savedKeys.add(String(message.key)); savingKeys.delete(String(message.key)); }
      if (message.url) { savedKeys.add(String(message.url)); savingKeys.delete(String(message.url)); }
      const cleanName = String(message.filename || 'Whempy_Videos')
        .replace(/^(?:Whempy|IntrovertStudio)_Videos\//i, '')
        .replace(/\.mp4$/i, '');
    ...
    if (message?.type === 'BRANDAI_RAW_VIDEO_BATCH_DOWNLOAD_FAILED') {
      ...
      console.warn('[Whempy Downloader] Video ZIP could not be created:', message.error);
      return;
    }
  ...
  // Whempy v1.9: surface download failures on the page with a one-click retry.
  function showFailureToast(message) {
    try {
      const id = 'studio-relay-download-failure-toast';
      ...
      const toast = appendElement(document.body || document.documentElement, 'div', {
        ...
        style: [
          ...
          'background:rgba(17,24,39,0.96)', 'border:1px solid rgba(248,113,113,0.55)', 'color:#fee2e2',
          'font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif', 'font-size:12.5px',
          'box-shadow:0 12px 36px rgba(0,0,0,0.55)', 'max-width:min(560px,92vw)'
        ].join(';')
      });
      const text = appendElement(toast, 'span', { text: 'Download failed: ' + String(message?.error || 'unknown error').slice(0, 160) });
      text.style.cssText = 'flex:1;line-height:1.35';
      const key = String(message?.key || message?.url || '');
      const video = extractedVideosPool.get(key) || Array.from(extractedVideosPool.values()).find(v => String(v?.url || '') === String(message?.url || ''));
      if (video) {
        const retry = appendElement(toast, 'button', { type: 'button', text: 'Retry' });
        retry.style.cssText = 'border:0;border-radius:8px;padding:6px 12px;background:#6366f1;color:#fff;font-weight:600;cursor:pointer';
        retry.addEventListener('click', () => {
          toast.remove();
          seenMedia.delete(key); seenMedia.delete(String(video.url || ''));
          cancelledMedia.delete(key); cancelledMedia.delete(String(video.url || ''));
          try {
            chrome.runtime.sendMessage({ type: 'BRANDAI_RAW_VIDEO_DOWNLOAD', manual: true, video: { ...video, pageUrl: location.href } }, () => { void chrome.runtime.lastError; });
          } catch (error) { console.warn('[Whempy] Retry failed:', error); }
        });
      }
      const close = appendElement(toast, 'button', { type: 'button', text: '✕', 'aria-label': 'Dismiss' });
      close.style.cssText = 'border:0;background:transparent;color:#fca5a5;cursor:pointer;font-size:14px';
